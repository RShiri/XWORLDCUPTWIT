...
# Every dashboard builder, run in dependency order — the exact set the regen CI and
# tools/check_2026_identity.py use, plus build_breaks.py (breaks.js is deployed too).
DASHBOARD_BUILDERS = [
    "build_match_details.py", "build_data.py", "build_players.py",
    "build_shots.py", "build_database.py", "build_player_lab.py", "build_breaks.py",
]


def _rebuild_dashboard_in_clone(tmpdir: str) -> bool:
    """Regenerate every dashboard output INSIDE the fresh clone, from the clone's
    own committed builders.

    This is the stale-checkout guard: the scrape machine's working copy can lag
    origin/main (features merged on GitHub, PC never pulled), and pushing its
    locally-built data.js/matches_detail/... used to regress the whole site until
    the regen-dashboard-data workflow flipped it back — one red bracket-parity run
    per match. Building from the clone's builders makes the deploy match what CI
    would regenerate by construction, whatever state the local checkout is in.
    Returns False (caller falls back to copying local outputs) if any builder fails.
    """
    for builder in DASHBOARD_BUILDERS:
        script = os.path.join(tmpdir, "wc2026_dashboard", builder)
        if not os.path.exists(script):
            log.warning("Dashboard rebuild: %s missing from the clone — falling back "
                        "to local outputs.", builder)
            return False
        r = _run([sys.executable, script], cwd=tmpdir, check=False)
        if r.returncode != 0:
            log.warning("Dashboard rebuild: %s exited %s — falling back to local "
                        "outputs.\n%s", builder, r.returncode, (r.stderr or "")[-2000:])
            return False
    return True
...
def push_match_update(png_path: str, match_id: str | None = None,
    ...
    *plus* the regenerated web-dashboard outputs (data.js, players.js, the match
    ...
    The dashboard outputs are rebuilt inside the fresh clone from the clone's own
    (origin/main) builders — see _rebuild_dashboard_in_clone. Copying the local
    working tree's outputs is only the fallback when that rebuild fails.
    ...
    with tempfile.TemporaryDirectory(prefix="xwctwit_") as tmpdir:
        ...
        # 2. the raw scraped match JSON — copied in BEFORE the rebuild so the
        #    clone's builders see the new match.
        _copy_file_into(tmpdir, os.path.join("wc2026", "matches", match_id + ".json"))

        # 3. the interactive-site outputs, regenerated in the clone by its own
        #    committed builders (covers data.js/players.js/shots.js/breaks.js,
        #    matches_detail/, database/, player_lab/ AND share/). Fallback: copy
        #    the local working tree's outputs — stale-checkout risk, but the site
        #    still updates and the regen CI corrects it.
        if not _rebuild_dashboard_in_clone(tmpdir):
            log.warning("Deploying LOCALLY-built dashboard outputs — if this checkout "
                        "lags origin/main the regen CI will have to correct the site.")
            ...
            _copy_tree_into(tmpdir, "wc2026_dashboard/player_lab")
            _copy_tree_into(tmpdir, "wc2026_dashboard/share")
